...
1. In case that it is not already supported, add the network on the **HARMONY_NETWORK** enum in [`src/config/networks/network.d.ts`](/src/config/networks/network.d.ts)
...
const rinkeby: NetworkConfig = {
  ...
  network: {
    id: HARMONY_NETWORK.<NETWORK_NAME>,
    backgroundColor: '',
    textColor: '',
    label: '',
...
```typescript
import { HARMONY_NETWORK, NetworkConfig } from 'src/config/networks/network.d'
...
const xDai: NetworkConfig = {
  ...
  disabledWallets: [WALLETS.TREZOR, WALLETS.LEDGER],
}
