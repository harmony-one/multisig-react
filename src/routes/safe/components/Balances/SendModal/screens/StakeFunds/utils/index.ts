...
export const ensResolver = createDecorator({
  ...
  updates: {
    validatorAddress: async (validatorAddress) => {
      try {
        const resolvedAddress = isValidEnsName(validatorAddress) && (await getAddressFromENS(validatorAddress))
...
export const formMutators: Record<string, Mutator<{ selectedMethod: { name: string } }>> = {
  ...
  setValidatorAddress: (args, state, utils) => {
    utils.changeValue(state, 'validatorAddress', () => args[0])
  },
